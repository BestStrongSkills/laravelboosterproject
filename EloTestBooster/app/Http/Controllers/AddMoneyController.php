...
class AddMoneyController extends HomeController
{
    ...
    public function winsPaymentWithpaypal(Request $request)
    {
            $wins_price=$request->wins_price;
            $winstotalData=$request->winstotalData;
            $wins_mode_statue=$request->wins_mode_statue;
            ...
            $item_1->setName('Item 1') 
                    ->setCurrency('USD')
                    ->setQuantity(1)
                    ->setPrice($request->get('wins_price'));
            ...
            $order->wins_price = $request->wins_price;
            $order->winstotalData = $request->winstotalData;
            $order->wins_mode_statue = $request->wins_mode_statue;
            $order->payment_id = $payment->getId();
            $order->save();
            Session::put('paypal_payment_id', $payment->getId());
...
//=========================gift payment=======================//
    public function giftpayWithPaypal()
    {
        return view('backend.memberArea_solo');
    }

    public function giftPayment_With_Paypal()
    {
        $amount=$request->amount;
        ...
        $redirect_urls = new RedirectUrls();
        $redirect_urls->setReturnUrl(URL::route('payment.gift_payment_status')) /** Specify return URL **/
                      ->setCancelUrl(URL::route('payment.gift_payment_status'));
